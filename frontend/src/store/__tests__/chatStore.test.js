...
describe('chatStore.js — ask() 方法触发链路', () => {
  ...
  it('调用 ask() 时添加用户消息到 messages', async () => {
    chatApi.sendChatMessageStream.mockResolvedValue(undefined)

    await store.ask({ message: '分析恒瑞医药' })

    const userMsg = store.messages.find(m => m.role === 'user')
    expect(userMsg).toBeDefined()
    expect(userMsg.content).toBe('分析恒瑞医药')
    expect(userMsg.createdAt).toBeTypeOf('number')
  })
  ...
  it('调用 ask() 时添加空的 assistant 消息', async () => {
    ...
    expect(store.messages.length).toBe(initialCount + 2) // user + assistant
  ...
  it('loading 状态在请求期间为 true', async () => {
    let resolveStream
    chatApi.sendChatMessageStream.mockReturnValue(
  ...
  it('传入 targets 但无 message 时，自动生成联合分析提示', async () => {
    chatApi.sendChatMessageStream.mockResolvedValue(undefined)
    ...
    const userMsg = store.messages.find(m => m.role === 'user')
    expect(userMsg.content).toBe('请对以下标的进行联合分析：恒瑞医药、长春高新')
  })
  ...
  it('传入 targets 和 message 时，在 message 前添加联合分析标签', async () => {
    ...
    const userMsg = store.messages.find(m => m.role === 'user')
    expect(userMsg.content).toBe('[联合分析：恒瑞医药、长春高新] 对比研发投入')
  })
  ...
  it('history 包含之前的所有消息（不含当前 assistant 消息）', async () => {
    ...
    // history 应该包含初始消息 + 第一轮的 user + assistant
    const historyRoles = payload.history.map(m => m.role)
    expect(historyRoles).toContain('assistant') // 初始欢迎消息
    expect(historyRoles).toContain('user') // 第一个问题
  })
  ...
  it('用户消息保留 targets 字段', async () => {
    chatApi.sendChatMessageStream.mockResolvedValue(undefined)

    const targets = [{ symbol: '600276', name: '恒瑞医药', type: 'stock' }]
    await store.ask({ message: '分析', targets })

    const userMsg = store.messages.find(m => m.role === 'user' && m.content.includes('分析'))
    expect(userMsg.targets).toEqual(targets)
  })
  ...
  it('多次调用 ask() 累积消息', async () => {
    ...
    expect(store.messages.length).toBe(initialCount + 6) // 每次 ask 添加 2 条消息
  })
