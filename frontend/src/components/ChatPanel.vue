  ...
  <div class="cp-root">
    ...
    <div class="cp-main">
      ...
      <div class="cp-messages" ref="msgListRef" @scroll="onMessagesScroll">
        ...
        >
          ...
          <div class="cp-bubble-wrap">
            ...
            <!-- Agent 推理轨迹 -->
            <AgentTrace
              v-if="msg.isAgent && (msg.agentTrace?.length || (isGenerating && i === chatStore.messages.length - 1))"
              :steps="msg.agentTrace || []"
              :sources="msg.agentSources || []"
              :loading="isGenerating && i === chatStore.messages.length - 1"
            />
            <!-- 模式与上下文 -->
            ...
            <!-- 气泡 -->
            <div
              v-if="shouldShowBubble(msg, i) && msg.role === 'assistant' && msg.isClarification"
              class="cp-bubble cp-bubble--clarification"
            >
              <div class="cp-clarification-question">{{ msg.content }}</div>
              <div v-if="msg.clarificationSuggestions?.length" class="cp-clarification-suggestions">
                <button
                  v-for="s in msg.clarificationSuggestions"
                  :key="s"
                  class="cp-clarification-chip"
                  :disabled="isGenerating"
                  @click="chatStore.answerClarification(s)"
                >{{ s }}</button>
              </div>
            </div>
            <div
              v-else-if="shouldShowBubble(msg, i) && msg.role === 'assistant'"
              class="cp-bubble md-body"
            >
              <template v-if="hasCharts(msg.content)">
                <template v-for="(seg, si) in parseMessageSegments(msg.content)" :key="si">
                  <div v-if="seg.type === 'text' && seg.content.trim()" v-html="renderMarkdown(seg.content)" />
                  <ReportChart v-else-if="seg.type === 'chart'" :config="seg.config" />
                </template>
              </template>
              <template v-else>
      ...
      <div class="cp-input-area">
        <!-- 澄清提示条 -->
        <div v-if="chatStore.pendingClarification" class="cp-clarification-bar">
          <span class="cp-clarification-bar-icon">❓</span>
        ...
        <ChatBox :loading="isGenerating" @submit="handleSubmit" />
      </div>
...
function parseMessageSegments(content) {
  ...
  while ((match = CHART_RE.exec(content)) !== null) {
    ...
    try {
      segments.push({ type: 'chart', config: JSON.parse(match[1]) })
    } catch {
...
onMounted(() => {
  ...
  window.addEventListener('chat-scroll-bottom', onScrollBottomEvent)
})

onUnmounted(() => {
  window.removeEventListener('chat-scroll-bottom', onScrollBottomEvent)
})
...
function isStreamingAssistant(msg) {
  return msg.role === 'assistant' && isGenerating.value && !msg.content
}
...
.cp-copy-btn:hover {
  color: var(--accent2);
  border-color: var(--accent2);
  background: rgba(75,169,154,0.06);
}
.cp-copy-btn.copied {
  color: var(--accent2);
  border-color: var(--accent2);
...
.cp-clarification-question {
  font-size: 14px;
  font-weight: 600;
  line-height: 1.6;
  margin-bottom: 10px;
}
.cp-clarification-suggestions {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
}
.cp-clarification-chip {
  font-size: 12px;
  font-weight: 600;
  padding: 4px 12px;
  border-radius: 20px;
...
.cp-clarification-chip:hover:not(:disabled) {
  background: #f59e0b;
  color: #fff;
  border-color: #f59e0b;
}
.cp-clarification-chip:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}

/* ── 澄清提示条 ── */
.cp-clarification-bar {
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 6px 12px;
...
.cp-clarification-bar-icon {
  font-size: 14px;
  flex-shrink: 0;
}
.cp-clarification-bar-text {
  flex: 1;
}
.cp-clarification-bar-dismiss {
  font-size: 11px;
  padding: 2px 8px;
  border-radius: 12px;
  border: 1px solid #f59e0b;
...
.cp-clarification-bar-dismiss:hover {
  background: #f59e0b;
  color: #fff;
}
