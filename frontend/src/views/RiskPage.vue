    ...
    <div class="page-header">
      ...
      <div class="header-controls">
        <input
          ...
          class="symbols-input"
          placeholder="股票代码，逗号分隔（如 testa,testb）"
          @keyup.enter="applySymbols"
        />
        <button class="apply-btn" @click="applySymbols">确定</button>
...
function applySymbols() {
  const s = symbolsInput.value.trim()
  if (s) {
    symbols.value = s
    loadData()
  }
...
.symbols-input {
  ...
  padding: 6px 12px; border-radius: 8px; font-size: 13px; width: 240px; transition: border-color .2s;
}
.symbols-input:focus { outline: none; border-color: var(--border-hl); }
.apply-btn {
  background: var(--accent2); color: #fff; border: none;
  padding: 6px 14px; border-radius: 8px; cursor: pointer; font-size: 13px; transition: background .2s;
}
.apply-btn:hover { background: #3d9688; }
.refresh-btn {
