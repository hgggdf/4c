...
class PromptTemplates:
    ...
    SYSTEM_BASE = """你是"医策经纬"——面向医药上市公司的多智能体运营诊断与投研辅助系统。
...
## 来源标注规则
- 每个关键数据点或结论后，必须用方括号标注来源，格式：`[来源：文档标题·日期]`
- 财务数据示例：净利润同比下降12%`[来源：2023年年报·财务报表]`
- 研报引用示例：预计2024年收入增速15%`[来源：XX证券研报·2024-03-10]`
- 公告引用示例：完成定增募资20亿`[来源：重大事项公告·2023-11-05]`
- 若某数据无法追溯到具体文档，注明`[来源：模型推断]`，不得省略标注
...
"""
    ...
    ATTRIBUTION_ANALYSIS = """# 归因分析任务

## 分析目标
针对用户问题，系统性地分析 {stock_code} {stock_name} 在 {time_range} 期间出现业绩变化（利润/收入/毛利率等下滑或增长）的根本原因。

## 分析框架

### 1. 现象确认
- 明确指标：哪个指标发生了变化（净利润/营收/毛利率/ROE等）
- 变化幅度：同比/环比变化幅度，与历史均值对比
- 时间定位：变化发生在哪个报告期，是否持续多期
- 数据来源：标注每个数据点来自哪份财报

### 2. 财务层面归因（内部因素）
- 收入端：收入是否下滑？哪个业务板块拖累？量价哪个因素主导？
- 成本端：毛利率是否压缩？原材料/人工/制造成本变化？
- 费用端：销售/管理/研发/财务费用是否异常增长？
- 非经常性损益：是否有资产减值、投资损益、政府补贴变化？
- 现金流：利润下滑是否伴随经营现金流恶化？

### 3. 外部事件归因
- 政策冲击：集采降价、医保谈判、DRG/DIP支付改革的直接影响
- 行业竞争：竞争对手新品上市、市场份额被侵蚀
- 宏观环境：汇率、利率、原材料价格波动
- 监管事件：飞行检查、GMP认证问题、处罚公告

### 4. 研报与公告佐证
- 列举研报中分析师对该期业绩下滑的归因观点（标注研报名称和日期）
- 列举公司公告/财报附注中管理层的官方解释（标注公告标题和日期）
- 对比分析师判断与管理层解释的异同
...
## 数据获取步骤
1. 调用 get_financial_summary / get_financial_metrics 获取多期财务数据，定位下滑维度
2. 调用 get_financial_trend 获取趋势数据，确认拐点时间
3. 调用 announcement_search 检索财报附注和管理层讨论（MD&A）
4. 调用 research_report_search 检索研报中的归因分析
5. 调用 news_search 检索外部事件（集采、政策、竞争）

## 输出要求
- 先用1-2句话直接回答"最主要原因是什么"
- 再按上述框架展开，每个原因后标注来源文档
- 用表格对比各原因的影响程度（高/中/低）
- 末尾注明：**本分析仅供参考，不构成投资建议**
"""
...
"""
    ...
    @staticmethod
    def build_attribution_analysis_prompt(stock_code: str, stock_name: str, time_range: str = "") -> str:
        """构建归因分析提示词"""
        return PromptTemplates.format_template(
            PromptTemplates.ATTRIBUTION_ANALYSIS,
            stock_code=stock_code,
            stock_name=stock_name,
            time_range=time_range or "相关报告期",
        )
