...
class GLMMinimalAgent:
    ...
    def run(
        ...
        *,
        ...
        cache_key = self._build_cache_key(question, stock_context, session_id=session_id)
    ...
    def _build_cache_key(self, question: str, stock_context: dict[str, Any] | None, *, session_id: int | None) -> str:
        stock_code = (stock_context or {}).get("stock_code") or "none"
        digest = md5(f"{question}|{stock_code}|{session_id or 0}".encode("utf-8")).hexdigest()[:16]
