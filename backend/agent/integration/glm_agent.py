...
class GLMMinimalAgent:
    ...
    def run(
        ...
        *,
        ...
        self._current_mode = state.resolved_mode
        ...
        state.company_list = list(entity_result.get("company_list") or [])
        ...
        if state.freshness_strategy != "insufficient":
            ...
            state.tool_plan = []
        dry_run = state.resolved_mode != "industry_compare"
    ...
    def _build_local_fallback(
        ...
        resolved_mode = getattr(self, "_current_mode", None)
        ...
        # report_generation 模式：在 answer 里嵌入可视化图表
        if resolved_mode == "report_generation":
            answer = self._build_report_with_charts(
    ...
    def _build_report_with_charts(
        ...
        lines: list[str] = []
        lines.append(f"# {stock_name} 投研报告\n")

        # 一、财务健康度
        lines.append("## 一、财务健康度\n")
        if analysis_summary:
            score = analysis_summary.get("total_score", "N/A")
            level = analysis_summary.get("level", "未知")
            lines.append(f"综合评分 **{score}** 分，评级 **{level}**。\n")
            ...
            if dims:
                ...
                radar_chart = {
                    "_title": "多维度评分雷达图",
                    "radar": {
                        "indicator": [{"name": n, "max": 100} for n in dim_names]
                    },
                    "series": [{
                        "type": "radar",
                        "data": [{"value": dim_scores, "name": stock_name}],
                        "areaStyle": {"opacity": 0.25}
                    }],
                    "legend": {"data": [stock_name]}
                }
                lines.append(f":::chart\n{_json.dumps(radar_chart, ensure_ascii=False)}\n:::\n")
            ...
            lines.append("暂无本地评分数据，建议补充财务数据后重新分析。\n")

        # 二、营收与利润趋势
        lines.append("## 二、营收与利润趋势\n")
        revenue_points = next((c for c in chart_context if c.get("metric") == "营业总收入"), None)
        profit_points = next((c for c in chart_context if c.get("metric") == "净利润"), None)

        if revenue_points or profit_points:
            dates, rev_vals, prof_vals = [], [], []
            if revenue_points:
                for p in revenue_points.get("points") or []:
                    dates.append(str(p.get("year") or p.get("period") or p.get("date") or ""))
                    rev_vals.append(float(p.get("value") or 0))
            if profit_points and not dates:
                for p in profit_points.get("points") or []:
                    dates.append(str(p.get("year") or p.get("period") or p.get("date") or ""))
            if profit_points:
                prof_vals = [float(p.get("value") or 0) for p in (profit_points.get("points") or [])]

            series = []
            if rev_vals:
                series.append({"name": "营业总收入", "type": "bar", "data": rev_vals, "yAxisIndex": 0})
            if prof_vals:
                series.append({"name": "净利润", "type": "line", "data": prof_vals, "yAxisIndex": 1, "smooth": True})
            ...
            if series:
                ...
                lines.append(f":::chart\n{_json.dumps(trend_chart, ensure_ascii=False)}\n:::\n")
        else:
            lines.append("暂无趋势数据。\n")
        ...
        lines.append("## 三、风险提示\n")
        if analysis_summary:
            weaknesses = analysis_summary.get("weaknesses") or []
