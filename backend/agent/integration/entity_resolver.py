...
def extract_stock_codes(text: str) -> list[str]:
    return re.findall(r"(?<!\d)(\d{6})(?!\d)", str(text or ""))


def extract_company_names_multi(text: str) -> list[str]:
    source = str(text or "")
    found: list[str] = []
    ...
    if found:
        return found
    pattern = r"([一-鿿]{2,12}(?:医药|生物|药业|制药|股份|医疗|科技|高新|集团))"
...
def extract_company_list(
    ...
    """从问题和 targets 中提取多公司列表，用于行业对比场景。"""
    ...
    for target in targets or []:
        if str(target.get("type") or "").strip() != "stock":
            continue
        code = str(target.get("symbol") or "").strip() or None
        name = str(target.get("name") or "").strip() or None
        if code and code not in seen_codes:
            seen_codes.add(code)
            result.append({"stock_code": code, "company_name": name, "source": "targets"})
        elif name and name not in seen_names:
            seen_names.add(name)
            result.append({"stock_code": None, "company_name": name, "source": "targets"})

    # 从问题文本提取股票代码
    for code in extract_stock_codes(question):
        if code not in seen_codes:
            seen_codes.add(code)
            result.append({"stock_code": code, "company_name": None, "source": "question"})

    # 从问题文本提取公司名称
    for name in extract_company_names_multi(question):
        if name not in seen_names:
            seen_names.add(name)
            result.append({"stock_code": None, "company_name": name, "source": "question"})
...
def resolve_entities(
    ...
    # 多公司列表：仅在识别到 2+ 家时填充
    company_list = extract_company_list(question, targets=targets)
    if len(company_list) >= 2:
        entity_result["company_list"] = company_list
