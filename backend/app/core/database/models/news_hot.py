...
# 向后兼容别名
# 使用方：stock_service.py, retrieval_service.py, sync.py, verify_vector_index.py 等
NewsRawHot = NewsHot
NewsStructuredHot = NewsHot
NewsRawArchive = NewsArchive
NewsStructuredArchive = NewsArchive

# 旧映射扩展表已废弃，设为 None 使调用方的 `if Model is not None` 守卫生效
NewsIndustryMapHot = None
NewsCompanyMapHot = None
IndustryImpactEventHot = None
NewsIndustryMapArchive = None
NewsCompanyMapArchive = None
IndustryImpactEventArchive = None
