...
# 向后兼容别名
# 使用方：stock_service.py, retrieval_service.py, sync.py, fast_backfill.py 等
...
# 旧事件扩展表已废弃，设为 None 使调用方的 `if Model is not None` 守卫生效
DrugApprovalHot = None
ClinicalTrialEventHot = None
CentralizedProcurementEventHot = None
RegulatoryRiskEventHot = None
