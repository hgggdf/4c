...
def chat_stream(request: ChatRequest, db: Session = Depends(get_db)):
	"""Kimi 流式对话（SSE）。"""
	# 有 session_id 时从数据库拉取历史，不依赖前端传递
	db_messages: list[dict] | None = None
	current_stock_code: str | None = None
	if request.session_id is not None:
		repo = ChatRepository(db)
		rows = repo.list_messages(request.session_id)
		# 只取最近 30 条，防止冷启动时把整个长会话历史塞入上下文
		db_messages = [{"role": r.role, "content": r.content} for r in rows[-30:]]
...
def delete_session(payload: ChatSessionModel, container: ServiceContainer = Depends(get_container)):
	result = service_result_response(container.chat.delete_session(build_request(ChatSessionRequest, payload)))
	get_session_memory_cache().invalidate(payload.session_id)
	return result
