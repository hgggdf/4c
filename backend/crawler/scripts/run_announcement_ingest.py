...
def run_ingest(stock_code: str, *, base_url: str, sync_vector_index: bool, timeout: int) -> dict:
    ...
    payload = build_announcement_package_payload(dataset, stock_code, sync_vector_index=sync_vector_index)
    status_code, response_body = _post_json(
    ...
    return {
        ...
        "raw_announcements_count": len(payload["raw_announcements"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch announcements and ingest through the existing announcement package API.")
