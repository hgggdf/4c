...
读取 staging/macro_phasec_live/macro_indicators_latest.json
调用 POST /api/macro-write/macro-indicators
注意：宏观 staging 顶层是 records，不是 payload
"""
...
def run_macro(job_id: str | None = None, dry_run: bool = False) -> dict:
    """
    执行 macro 入库
    """
    config = get_config()
    ...
    if job_id:
        ...
        manifest = next((m for m in manifests if m.data_category == "macro"), None)

    if manifest is None:
        return {"success": False, "error": "未找到 macro manifest"}
    ...
    # 3. 调用 API（宏观不走 news-package，直接调 macro-indicators）
    api_payload = {
        "items": records,
    }
...
def main():
    parser = argparse.ArgumentParser(description="macro 入库脚本")
    ...
    result = run_macro(job_id=args.job_id, dry_run=args.dry_run)
