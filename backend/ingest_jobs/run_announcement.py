...
"""
run_announcement.py
announcement_raw 链入库脚本

读取 staging/announcement_package.json
调用 POST /api/ingest/announcement-package
只写入 raw_announcements 字段
"""
...
def run_announcement(job_id: str | None = None, dry_run: bool = False) -> dict:
    """
    执行 announcement_raw 入库
    """
    config = get_config()
    http = HttpClient()
    ...
    if job_id:
        ...
        manifest = next((m for m in manifests if m.data_category == "announcement_raw"), None)

    if manifest is None:
        return {"success": False, "error": "未找到 announcement_raw manifest"}
...
def main():
    parser = argparse.ArgumentParser(description="announcement_raw 入库脚本")
    ...
    result = run_announcement(job_id=args.job_id, dry_run=args.dry_run)
