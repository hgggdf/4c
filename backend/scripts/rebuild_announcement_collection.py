...
from app.knowledge.sync import sync_announcements
...
def _collect_counts() -> dict[str, dict]:
    ...
    try:
        return {
            doc_type: _safe_collection_count(client, collection_name)
...
def _backfill_announcements() -> int:
    with SessionLocal() as db:
        try:
            backfilled = int(sync_announcements(db, is_hot=True))
            db.commit()
            return backfilled
        except Exception:
            db.rollback()
...
def _wait_for_reopenable_collection(expected_min_count: int) -> dict:
    ...
    raise RuntimeError(
        "announcement collection is still not reopenable in a fresh process: "
...
def run_repair(*, skip_backfill: bool = False) -> dict:
    ...
    if not skip_backfill:
        backfilled = _backfill_announcements()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Rebuild only the damaged announcement Chroma collection and backfill announcement vectors.")
    parser.add_argument("--skip-backfill", action="store_true", help="Only drop and recreate the announcement collection without backfilling vectors")
