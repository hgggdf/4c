...
class StockDailySummaryYearly(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_stock_daily_summary_yearly_code", "stock_code", "year"),
    )
    ...
    avg_open: Mapped[Decimal | None] = mapped_column(DECIMAL(12, 4), nullable=True)
    avg_close: Mapped[Decimal | None] = mapped_column(DECIMAL(12, 4), nullable=True)
    max_high: Mapped[Decimal | None] = mapped_column(DECIMAL(12, 4), nullable=True)
    min_low: Mapped[Decimal | None] = mapped_column(DECIMAL(12, 4), nullable=True)
    avg_volume: Mapped[int | None] = mapped_column(Integer, nullable=True)
    year_return_ratio: Mapped[Decimal | None] = mapped_column(DECIMAL(10, 6), nullable=True)
...
class IndustryNewsSummaryMonthly(Base):
    __tablename__ = "industry_news_summary_monthly"
    __table_args__ = (
        UniqueConstraint("industry_code", "year_month", name="uk_industry_news_summary_monthly"),
        Index("idx_industry_news_summary_monthly_industry", "industry_code", "year_month"),
    )
