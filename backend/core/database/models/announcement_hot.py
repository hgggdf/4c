...
from sqlalchemy import JSON, Date, DateTime, DECIMAL, ForeignKey, Index, Integer, SmallInteger, String, Text, UniqueConstraint
...
class AnnouncementStructuredHot(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_ann_struct_hot_code", "stock_code"),
        Index("idx_ann_struct_hot_category", "category"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    announcement_id: Mapped[int] = mapped_column(Integer, ForeignKey("announcement_raw_hot.id"), nullable=False)
    stock_code: Mapped[str] = mapped_column(String(20), ForeignKey("company_master.stock_code"), nullable=False)
...
class DrugApprovalHot(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_drug_approval_hot_source", "source_announcement_id"),
    )
    ...
    drug_stage: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    market_scope: Mapped[str | None] = mapped_column(String(100), nullable=True)
    source_announcement_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("announcement_raw_hot.id"), nullable=True)
    source_type: Mapped[str | None] = mapped_column(String(50), nullable=True)
    source_url: Mapped[str | None] = mapped_column(String(500), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now, nullable=False)
...
class ClinicalTrialEventHot(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_clinical_hot_source", "source_announcement_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    stock_code: Mapped[str] = mapped_column(String(20), ForeignKey("company_master.stock_code"), nullable=False)
    drug_name: Mapped[str | None] = mapped_column(String(200), nullable=True)
    trial_phase: Mapped[str | None] = mapped_column(String(50), nullable=True)
...
class CentralizedProcurementEventHot(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_procurement_hot_source", "source_announcement_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    stock_code: Mapped[str] = mapped_column(String(20), ForeignKey("company_master.stock_code"), nullable=False)
    drug_name: Mapped[str | None] = mapped_column(String(200), nullable=True)
    procurement_round: Mapped[str | None] = mapped_column(String(100), nullable=True)
    bid_result: Mapped[str | None] = mapped_column(String(50), nullable=True)
    price_change_ratio: Mapped[Decimal | None] = mapped_column(DECIMAL(10, 4), nullable=True)
    event_date: Mapped[date | None] = mapped_column(Date, nullable=True)
...
class RegulatoryRiskEventHot(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_reg_risk_hot_source", "source_announcement_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    stock_code: Mapped[str] = mapped_column(String(20), ForeignKey("company_master.stock_code"), nullable=False)
    risk_type: Mapped[str | None] = mapped_column(String(100), nullable=True)
    event_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    risk_level: Mapped[str | None] = mapped_column(String(50), nullable=True)
    summary_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    source_announcement_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("announcement_raw_hot.id"), nullable=True)
