...
def _run_tool(tool_name: str, input_data: dict[str, Any]) -> tuple[bool, Any, str | None]:
    ...
    if tool_name == "industry_compare":
        company_list = input_data.get("company_list") or []
        stock_codes = [c["stock_code"] for c in company_list if c.get("stock_code")]

        # 对只有公司名称的条目，尝试解析出股票代码
        for c in company_list:
            if c.get("stock_code") or not c.get("company_name"):
                continue
            try:
                resolved = company_tools.resolve_company_from_text(c["company_name"])
                if resolved:
                    code = resolved[0].get("stock_code")
        ...
        if len(stock_codes) < 2:
            return False, None, "行业对比至少需要2家公司的股票代码"
        metrics = input_data.get("compare_metrics") or [
            "gross_margin", "net_margin", "roe", "rd_ratio", "revenue_growth"
        ]
