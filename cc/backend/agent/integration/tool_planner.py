...
def _base_input(
    ...
    if company_list:
        base["company_list"] = company_list
    return base
...
def build_tool_plan(
    ...
    base_input = _base_input(question, selected_mode, company_entity, drug_entity, industry_entity, time_range, company_list)
    ...
    if mode == "attribution_analysis":
        add("company_basic_info", purpose="获取公司基础信息", required=True, data_source_type="公司", can_score=False, freshness_value="local")
        add("financial_metrics", purpose="获取财务指标以识别下滑维度", required=True, data_source_type="财报", can_score=True, freshness_value="local")
        add("financial_trend", purpose="获取多期财务趋势以定位拐点", required=True, data_source_type="财报", can_score=True, freshness_value="local")
        add("announcement_search", purpose="检索财报附注和重大公告", required=True, data_source_type="公告", can_score=False, freshness_value="local")
        add("research_report_search", purpose="检索研报中的归因分析", required=True, data_source_type="研报", can_score=False, freshness_value="local")
