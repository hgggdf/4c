<template>
  <div class="news-panel">
    ...
    <div class="news-drag-hint">
      <span class="news-drag-hint-icon">💬</span>
      拖拽新闻到右侧对话框可快速分析
    </div>
    ...
    <div class="news-list" v-if="!loading && news.length">
      <div
        ...
        :class="{ 'news-item--dragging': draggingId === item.id }"
        @click="openDetail(item)"
      >
        <div class="news-drag-handle" title="拖到右侧对话框分析">⠿</div>
...
function onDragStart(evt, item) {
  ...
  evt.dataTransfer.setData('application/json', JSON.stringify({
    ...
    id: item.id,
    title: item.title,
    summary: item.summary_text || item.content || '',
...
.news-drag-hint {
  ...
  border-bottom: 1px solid rgba(75,169,154,0.15);
  flex-shrink: 0;
}
.news-drag-hint-icon { font-size: 13px; }
...
.news-item:hover .news-drag-handle { opacity: 1; }
